...
angular.module('game.directives.tile', [])
  .controller("TileCtrl", function() {
    console.log("Hello from tile controller");
  ...
  .directive("tile", function() {
    return {
      ...
      link: function(scope, element, attrs, model, tile) {
...
angular.module('game.factories.tile', [])
  ...
  .factory("Tile", function(TILE_WIDTH, TILE_HEIGHT, Z_LEVEL_ADJUSTMENT, Z_ROW_ADJUSTMENT, Z_OFFSET) {
    return function Tile(position, options) {
      console.log("Hello from Tile Factory");
      var tile = this;
      ...
      tile.setPosition = function(position) {
        ...
        tile.position = {
          ...
          z: position.z || tile.position.x
        }

        // Update the DOM translated position (What the fuck was I thinking?)
        tile.translated = {
          // Left value
          x: TILE_WIDTH * position.x + (position.y * TILE_WIDTH),
          // Top value
          y: TILE_HEIGHT * position.x * -1 + (position.y * TILE_HEIGHT) - (position.z * Z_OFFSET),
